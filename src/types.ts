...
/**
 * The six severity levels supported by the logger, ordered from lowest to
 * highest: `trace < debug < info < warn < error < fatal`.
 *
 * Numeric weights are exposed on `LogRecord.levelValue` and are stable once
 * v1 is released.
 */
 ...
 *
 * Values may be any JSON-compatible type. Top-level `Error` values are
 * serialized to plain objects by the logger before transport delivery.
 */
 ...
 *
 * The logger creates exactly one record per enabled log call. Transports
 * receive the record after level filtering and context serialization.
 */
export interface LogRecord {
  ...
  /** Numeric weight of `level`. Stable across v1 releases. */
 ...
 *
 * `log` is called synchronously for every record that passes level filtering.
 * Built-in transports must not throw; the logger wraps all transport calls in
 * try/catch and routes failures through `onTransportError`.
 */
 ...
 *
 * Receives the thrown value and the record that triggered the failure.
 * Any exception thrown by this callback is silently swallowed by the logger.
 */
...
export interface CreateLoggerOptions {
   ...
   * before context is accessed. Defaults to `"info"`.
   */
  readonly level?: LogLevel;
  /** Transport to receive emitted records. Required. */
  readonly transport: Transport;
  /**
   * Key-value pairs merged into every record's context.
   * Per-call context wins on key collision. Defaults to `{}`.
   */
  readonly bindings?: LogContext;
  /**
   * Clock used to produce `record.time`. Defaults to `() => new Date()`.
   * Override in tests for deterministic timestamps.
   */
  readonly clock?: () => Date;
  /**
   * Called when the transport throws. Receives the error and the record that
   * triggered it. Exceptions thrown by this callback are swallowed.
   */
 ...
 *
 * Each level method checks `isLevelEnabled` before touching the context
 * argument, so disabled calls impose no observable side effects.
 */
export interface Logger {
  ...
  /**
   * Return a new logger that inherits this logger's transport, level, clock,
   * and error handler, with `bindings` merged on top of the parent's bindings.
...
export interface BrowserTransportOptions {
  /**
   * CSS style strings applied to the level badge via `%c` in `console` calls.
   * Provide only the levels you want to override; unspecified levels use the
   * transport's built-in defaults.
   */
...
export interface ConsoleTransportOptions {
  /**
   * Whether to emit ANSI color codes in the output. Defaults to `true`.
   * Set to `false` for CI environments or when piping output to a file.
   */
...
export interface StructuredTransportOptions {
  /**
   * When `true`, serializes the payload to a JSON string before passing it to
   * the console method. When `false` (default), passes a plain object so that
   * Cloudflare Workers Logs can extract and index individual fields.
   */
...
export interface CaptureTransport extends Transport {
  /**
   * Read-only ordered list of all records received since the last `clear()`.
   * Returns an immutable snapshot; mutating the returned value does not affect
   * internal storage.
   */
