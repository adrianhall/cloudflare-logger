/**
 * @file Tests for `src/logger.ts`.
 *
 * Covers all required cases from ENG_SPEC.md §20.1:
 ...
 *  - Nested errors are not serialized unless top-level.
...
import * as sut from "../../src/logger.js";
import type { LogRecord, Transport } from "../../src/types.js";
...
describe("createLogger()", () => {
  ...
  describe("default level", () => {
    it("defaults to info level", () => {
      ...
      const logger = sut.createLogger({ transport, clock });
  ...
  describe("clock", () => {
    ...
    it("default clock produces an ISO timestamp string", () => {
      const { transport, records } = makeCapture();
      const logger = sut.createLogger({ transport });
      logger.info("msg");
  ...
  describe("level method record fields", () => {
    ...
    for (const { method, expectedValue } of levels) {
      it(`${method}() sets record.level to "${method}"`, () => {
        ...
        const logger = sut.createLogger({ transport, clock, level: "trace" });
        logger[method]("test");
  ...
  describe("level filtering", () => {
    it("emits records at exactly the configured level", () => {
      const { transport, records } = makeCapture();
      const logger = sut.createLogger({ transport, clock, level: "warn" });
  ...
  describe("context merging", () => {
    ...
    it("bindings appear in record.context", () => {
      const { transport, records } = makeCapture();
      const logger = sut.createLogger({ transport, clock, bindings: { service: "api" } });
      logger.info("msg");
    ...
    it("per-call context wins over bindings on key collision", () => {
      const { transport, records } = makeCapture();
      const logger = sut.createLogger({ transport, clock, bindings: { key: "binding" } });
    ...
    it("bindings and per-call context are both present when keys differ", () => {
      const { transport, records } = makeCapture();
      const logger = sut.createLogger({ transport, clock, bindings: { a: 1 } });
      logger.info("msg", { b: 2 });
    ...
    it("omitting per-call context still includes bindings", () => {
      const { transport, records } = makeCapture();
      const logger = sut.createLogger({ transport, clock, bindings: { x: "y" } });
  ...
  describe("input immutability", () => {
    it("does not mutate the bindings object", () => {
      const bindings = { a: 1 };
      const { transport } = makeCapture();
      const logger = sut.createLogger({ transport, clock, bindings });
      logger.info("msg", { b: 2 });
  ...
  describe("child loggers", () => {
    it("child records include child bindings", () => {
      const { transport, records } = makeCapture();
      const parent = sut.createLogger({ transport, clock });
    ...
    it("child merges parent bindings with child bindings", () => {
      const { transport, records } = makeCapture();
      const parent = sut.createLogger({ transport, clock, bindings: { service: "api" } });
      const child = parent.child({ component: "Widget" });
      child.info("msg");
      expect(records[0]?.context["service"]).toBe("api");
      expect(records[0]?.context["component"]).toBe("Widget");
    ...
    it("child bindings override parent bindings on collision", () => {
      const { transport, records } = makeCapture();
      const parent = sut.createLogger({ transport, clock, bindings: { key: "parent" } });
    ...
    it("parent logger does not pick up child bindings", () => {
      ...
      const parent = sut.createLogger({ transport, clock });
      const child = parent.child({ component: "Widget" });
      // emit from parent, then child
      parent.info("from parent");
      child.info("from child");
      // first record (from parent) should have no component key
      expect(records[0]?.context["component"]).toBeUndefined();
      // second record (from child) should have it
    ...
    it("child shares the same level as the parent", () => {
      const { transport, records } = makeCapture();
      const parent = sut.createLogger({ transport, clock, level: "warn" });
      const child = parent.child({ c: 1 });
  ...
  describe("error serialization", () => {
    ...
    it("does not serialize a nested error inside a context value", () => {
      ...
      // The wrapper object is not an Error, so it should not be serialized.
  ...
  describe("transport error isolation", () => {
    it("does not throw when transport.log throws", () => {
      ...
      const logger = sut.createLogger({ transport: throwingTransport, clock });
    ...
    it("does not call transport for disabled log levels", () => {
      ...
      const logger = sut.createLogger({ transport: countingTransport, clock, level: "info" });
      logger.debug("suppressed");
      logger.trace("suppressed");
    ...
    it("silently drops transport errors when no onTransportError is provided", () => {
      ...
      const logger = sut.createLogger({ transport: throwingTransport, clock });
      // Should not throw and should not require onTransportError.
      expect(() => logger.info("msg")).not.toThrow();
    });
  ...
  describe("record isolation", () => {
    ...
    it("mutating record.context after delivery does not affect subsequent records", () => {
      ...
      const logger = sut.createLogger({ transport, clock });
      logger.info("first", { x: 1 });
      // Forcibly mutate the delivered record context.
      (records[0]!.context as Record<string, unknown>)["x"] = 99;
      logger.info("second", { x: 2 });
      // Second record should still have x: 2.
      expect(records[1]?.context["x"]).toBe(2);
    });
  });
