/**
 * @file Tests for `src/internal/safe-json.ts`.
 *
 * Covers `replaceNonJsonValue()`:
 *  - `bigint` → `"<n>n"` string.
 *  - `symbol` → `"Symbol(description)"` string.
 *  - Named function → `"[Function name]"` string.
 *  - Anonymous function → `"[Function (anonymous)]"` string.
 *  - All other values (string, number, boolean, null, object, array,
 *    undefined) are returned unchanged — the default return branch (line 42).
 *
 * Covers `safeStringify()`:
 *  - Plain JSON-compatible values round-trip correctly.
 *  - Circular references are replaced with `"[Circular]"`.
 *  - `bigint` values are rendered as `"<n>n"`.
 *  - Symbols are rendered as `"Symbol(description)"`.
 *  - Functions are rendered as `"[Function name]"` or `"[Function (anonymous)]"`.
 *  - Arrays containing non-JSON values are handled.
 *  - `undefined` at the top level returns the string `"undefined"`.
 *  - A throwing getter does not propagate an exception.
 *
 * Note: both exports are internal helpers and are not part of the public
 * package API. They are tested here directly because the project convention
 * allows internal tests in the `node` Vitest project.
 */

import { describe, expect, it } from "vitest";
import * as sut from "../../src/internal/safe-json.js";
...
describe("replaceNonJsonValue()", () => {
  ...
  it("converts an arrow function with an inferred name", () => {
    ...
    // Inferred name is "arrowFn" in V8; the exact name is environment-dependent
    // so assert only the prefix shape rather than a hardcoded name.
    expect(sut.replaceNonJsonValue(arrowFn)).toMatch(/^\[Function /);
  });

  // ── default return — all other value types returned unchanged ─────────────
...
describe("safeStringify()", () => {
  ...
  it("serializes an anonymous arrow function as '[Function (anonymous)]'", () => {
    // Assign to a variable so we can strip the inferred name.
    const fn = Object.defineProperty(() => undefined, "name", { value: "" });
  ...
  // ── mixed non-JSON values in arrays ─────────────────────────────────────
