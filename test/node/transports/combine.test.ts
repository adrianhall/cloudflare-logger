/**
 * @file Tests for `src/transports/combine.ts`.
 *
 * Covers all required cases from ENG_SPEC.md §20.1 (Combine transport):
 *  - Forwards in order.
...
import * as sut from "../../src/transports/combine.js";
...
describe("combineTransports()", () => {
  describe("forwarding", () => {
    it("forwards a record to a single transport", () => {
      ...
      const combined = sut.combineTransports(transport);
    ...
    it("forwards a record to multiple transports in order", () => {
      ...
      const combined = sut.combineTransports(t1, t2, t3);
    ...
    it("passes the exact same record object to each transport", () => {
      ...
      sut.combineTransports(t1, t2).log(record);
    ...
    it("works with zero transports", () => {
      const combined = sut.combineTransports();
  ...
  describe("single child throws", () => {
    it("throws the child error after attempting all transports", () => {
      ...
      const combined = sut.combineTransports(throwing, good);

      expect(() => combined.log(makeRecord())).toThrow("first error");
      // The good transport must still have been called.
      expect(received).toHaveLength(1);
    });

    it("throws the child error when the only transport throws", () => {
      const combined = sut.combineTransports(makeThrowing("only error"));
    ...
    it("still calls subsequent transports even when an earlier one throws", () => {
      ...
      const combined = sut.combineTransports(good1, makeThrowing("middle"), good2);
  ...
  describe("multiple children throw", () => {
    it("throws an AggregateError when two children throw", () => {
      const combined = sut.combineTransports(makeThrowing("error one"), makeThrowing("error two"));
  ...
  describe("logger-level error isolation", () => {
    it("routes combined failure through onTransportError without crashing", () => {
      ...
      const combined = sut.combineTransports(makeThrowing("from combine"), makeCapture().transport);
    ...
    it("captures records from good transports even when another throws", () => {
      ...
      const combined = sut.combineTransports(makeThrowing("oops"), capture);
