 ...
 *
 * All logger state is captured in the closure; no class is used. Both root
 * loggers and child loggers are created through this function, which keeps
 * the child creation path identical to the root path.
 *
...
function makeLogger(level, transport, bindings, clock, onTransportError) {
    ...
    /**
     * Returns `true` when records at `candidate` severity will be emitted.
     * Used both for the public `isLevelEnabled` method and internally before
     ...
     *
     * Exits immediately when `logLevel` is below the configured minimum so that
     * disabled calls never access the `context` argument. When enabled, merges
     * bindings with per-call context, serializes top-level `Error` values, builds
     * the `LogRecord`, and delivers it to the transport inside a try/catch.
     *
    ...
    function emit(logLevel, message, context) {
        ...
        const merged = context !== undefined ? { ...bindings, ...context } : { ...bindings };
    ...
    return {
        ...
        child(childBindings) {
            ...
            return makeLogger(level, transport, { ...bindings, ...childBindings }, clock, onTransportError);
        }
    };
}
/**
...
export function createLogger(options) {
    const { transport, level = "info", bindings = {}, clock = () => new Date(), onTransportError } = options;
