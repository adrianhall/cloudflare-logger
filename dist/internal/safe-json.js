 ...
 */
/**
 * A stable placeholder emitted when `JSON.stringify` itself throws
 * unexpectedly (e.g. a getter that throws mid-serialization after the
 * circular-reference check has passed).
 */
 ...
 */
export function replaceNonJsonValue(value) {
...
export function safeStringify(value) {
    ...
    try {
        return JSON.stringify(value, function replacer(_key, val) {
            // Handle non-JSON primitives before the circular-ref check so that
            // symbols and functions (which are objects in some host environments) are
            // caught early.
