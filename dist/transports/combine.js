/**
 * @file Combine transport for `@adrianhall/cloudflare-logger`.
 *
 * `combineTransports()` fans out each record to multiple child transports in
 * declaration order. It attempts every child even when an earlier one throws,
 * then re-throws the collected failures so the logger-level try/catch can
 * route them through `onTransportError`.
 *
 * Failure semantics:
 ...
 */
export function combineTransports(...transports) {
    return {
        log(record) {
            const errors = [];
